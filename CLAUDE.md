...
- `hooks/hooks.json` + `hooks/ratatoskr-session-start.sh` — Claude Code plugin **SessionStart hook**. Activation-only: it injects a short instruction (via `additionalContext`) so the agent applies the skill from the first turn and re-asserts it after compaction; it encodes no rules of its own, so the self-containment rule holds — all behavior still lives in `SKILL.md`. Claude Code only.
